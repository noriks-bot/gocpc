...
class GoogleAdsClient {
  ...
  async listCampaigns(customerId, dateRange = 'LAST_30_DAYS') {
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        budgetResource: r.campaign?.campaignBudget,
        budgetMicros,
        dailyBudget: budgetMicros / 1_000_000,
  ...
  async updateCampaignStatus(customerId, campaignId, status) {
    ...
    const allowed = ['ENABLED', 'PAUSED', 'REMOVED'];
    if (!allowed.includes(status)) throw new Error(`Invalid status: ${status}`);
    return this.mutate(customerId, `/campaigns:mutate`, {
      operations: [{
        update: {
  ...
  async updateCampaignBudget(customerId, budgetResource, dailyBudgetEur) {
    // budgetResource is e.g. "customers/123/campaignBudgets/456"
    if (!budgetResource) throw new Error('budgetResource required');
    const amountMicros = String(Math.round(Number(dailyBudgetEur) * 1_000_000));
    return this.mutate(customerId, `/campaignBudgets:mutate`, {
      operations: [{
        update: {
